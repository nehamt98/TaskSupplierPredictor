...
import toml
import json
from src.utils import load_data, define_model
import os
...
def held_out_val(model_type):
    ...
    TestGroup = pd.read_csv(os.path.join(base_path, "data", "processed", "TestGroup.csv"))

    # Add the predicted cost for the tasks in Test Group
    y_pred = model.predict(X_test)
    y_pred = pd.DataFrame(y_pred, columns = ["Predicted Cost"])
    y_pred.index = X_test.index
    TestGroupDf = final_df[final_df['Task ID'].isin(TestGroup['Task ID'])][['Task ID','Supplier ID','Cost']]
